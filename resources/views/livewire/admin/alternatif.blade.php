<div>
        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th class="border-gray-200">Nama Alternatif</th>
                    <th class="border-gray-200">Kode Alternatif</th>
                ...
                <tr>
                    <td>
                        <a href="#" class="fw-bold">
                            {{ ($loop->index) + (($alternatifs->currentPage() - 1) * $alternatifs->perPage()) + 1 }}
                        </a>
                    </td>
                    <td>
                        <span class="fw-normal">{{ $alternatif->nama_alternatif }}</span>
                    </td>
                    <td>
                        <span class="fw-normal">{{ $alternatif->kode_alternatif }}</span>
                    </td>
                    <td class="text-end">
                        <a href="#" class="btn btn-info btn-sm btn-rounded" wire:click.prevent="edit({{ $alternatif->id }})" data-bs-toggle="modal" data-bs-target="#modalEdit">Edit</a>
                        <a href="#" wire:click.prevent="hapus({{ $alternatif->id }})" class="btn btn-danger btn-sm btn-rounded">Delete</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
        </table>
        <div class="mt-2">{{ $alternatifs->links() }}</div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content ">
                <div class="modal-header">
                    <h2 class="h6 modal-title">Tambah Alternatif</h2>
                ...
                <form>
                    ...
                    <div class="modal-body">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control @error('nama_alternatif') is-invalid @enderror" wire:model="nama_alternatif">
                            @error('nama_alternatif') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
                        <div class="form-group mb-3">
                            <label class="form-label">Kode Alternatif</label>
                            <input type="text" class="form-control @error('kode_alternatif') is-invalid @enderror" wire:model="kode_alternatif">
    ...
    <div wire:ignore.self class="modal fade" id="modalEdit" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content ">
                <div class="modal-header">
                    <h2 class="h6 modal-title">Update Alternatif</h2>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form>
                    @csrf
